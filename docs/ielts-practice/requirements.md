...
| Content creator | Chỉ xem danh sách, chi tiết và version; không có quyền mutation |
...
| FR-04 | Must | Chi tiết đề trả content đã redaction theo union của kỹ năng. | Không trả `acceptedAnswers`, `correctAnswer` hoặc system prompt/config nội bộ. | AC-04, AC-05 |
...
| FR-11 | Must | Writing/Speaking lưu submission nhưng không chấm trong MVP. | Submit chuyển `submitted`; không enqueue grading job và không sinh band/feedback. | AC-15 |
...
stateDiagram-v2
    ...
    submitted --> graded: chỉ Listening/Reading objective grading
```
...
|---|---|---|---|
| NFR-01 | Must | P95 list/detail ≤500 ms, start/save/submit ≤800 ms, không tính upload media. | AC-25 |
...
| NFR-09 | Must | Attempt, essay, transcript và audio Speaking được lưu vĩnh viễn; không có TTL hoặc cleanup job trong MVP. | AC-15, AC-30 |
...
- Attempt, essay, transcript và audio Speaking được lưu vĩnh viễn; không cấu hình TTL, hard delete scheduler hoặc lifecycle xóa storage trong MVP.
- Archive nội dung không xóa attempt/media đã nộp.
- Mọi publish/archive/rollback ghi audit log với actor, before/after summary và timestamp UTC.
