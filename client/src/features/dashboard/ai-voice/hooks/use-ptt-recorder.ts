...
export const usePttRecorder = (): UsePttRecorderReturn => {
	...
	const startRecording = useCallback(async (): Promise<boolean> => {
		...
		if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === 'undefined') {
			toast.error('Trình duyệt chưa hỗ trợ ghi âm microphone.');
			return false;
		}
		...
		try {
			...
			recorder.onstop = () => {
				...
				if (durationMs < MIN_DURATION_MS) {
					toast.warning('Vui lòng nói lâu hơn một chút.');
					resolver?.(null);
					releaseStream();
			...
			autoStopTimerRef.current = window.setTimeout(() => {
				if (mediaRecorderRef.current?.state === 'recording') {
					toast.warning('Đã tự dừng ghi âm sau 30 giây.');
	...
	return useMemo(() => ({
		...
		stopRecording,
		reset,
	}), [isRecording, reset, startRecording, stopRecording]);
};
