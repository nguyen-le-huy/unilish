...
// NOTE: This file should match server/src/utils/game-level.ts logic exactly.
...
// Build the table based on the curve
let cumulativeXp = 0;
for (let lvl = 1; lvl <= MAX_LEVEL; lvl++) {
    XP_TO_REACH_LEVEL[lvl] = cumulativeXp; // XP to reach Level 1 is 0.
...
export const GameLevelUtils = {
    ...
    calculateLevelFromXp(totalXp: number) {
        ...
        for (let i = 1; i <= MAX_LEVEL; i++) {
            if (totalXp >= XP_TO_REACH_LEVEL[i]) {
                level = i;
            } else {
                break;
            }
        ...
        if (level >= MAX_LEVEL) {
            return {
                level: MAX_LEVEL,
                currentLevelXp: totalXp - XP_TO_REACH_LEVEL[MAX_LEVEL], // Surplus
                nextLevelThreshold: getXpReqForLevel(MAX_LEVEL),
                progressPercent: 100,
                isMaxLevel: true
            };
        }

        const currentLevelStartTotalXp = XP_TO_REACH_LEVEL[level];
        const nextLevelStartTotalXp = XP_TO_REACH_LEVEL[level + 1];
    ...
    */
    getTotalXpToReachLevel(level: number): number {
        if (level <= 1) return 0;
        if (level > MAX_LEVEL) return XP_TO_REACH_LEVEL[MAX_LEVEL];
